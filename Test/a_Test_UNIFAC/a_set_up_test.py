...
def load_Experiments(name_group, fit_extrapolation):
    ...
    path_empty = 'Data_test/01_Experiments_'
    path_to_ctc2 = '.xlsx'
...
def load_Data(name_group, model):
    ...
    Solutes = pd.read_excel('Data_test/01_Molecules.xlsx', sheet_name= name_group, index_col ="Material")
    
    # load information about the solvents
    # Columns: Material, Formula Smiles, CAS, Source MPT, MPT °C, Source HFUS, HFUS J/mol
    Solvents = pd.read_excel('Data_test/01_Molecules.xlsx', sheet_name= 'Solvents', index_col ="Material")
    ...
    if model == 'COSMOSAC':
        ...
        path_empty = 'Data_test/01_StructuralInformation_'
        path_to_ctc2 = '.xlsx'
        path = path_empty + str(name_group) + path_to_ctc2
        structural_information = pd.read_excel(path, delimiter = ' ', header = 0, index_col ="Name")
        
        # combine output
        Data = [Solutes,Solvents,structural_information,structural_information]  
...
def load_Database(name_group, model):
    ...
    database = {}
    path_empty = 'Data_test/01_database_'
    path_to_ctc2 = '.xlsx'
