...
def load_Experiments(name_group, fit_extrapolation):
    ...
    path_empty = 'Adenventure\Handler\MixComp\Benchmark\Data\Experiments_'
    path_to_ctc2 = '.xlsx'
...
'''
In this function, the order for the consecutive fit is imported
Input:
        - name_group: group of solutes so that the right file can be loaded
Output:
        - Order_Fit: order in which the fit-parameter are to be fitted consecutivly
'''    
def load_OrderOfFit(name_group):
    # determine the order of fit for consecutive fitting
    path_empty = 'Adenventure\Handler\MixComp\Benchmark\Data\OrderOfFit_'
    path_to_ctc2 = '.xlsx'
    path = path_empty + str(name_group) + path_to_ctc2
...
def load_Data(name_group, model):
    ...
    Solutes = pd.read_excel('Adenventure\Handler\MixComp\Benchmark\Data\Molecules.xlsx', sheet_name= name_group, index_col ="Material")
    
    # load information about the solvents
    # Columns: Material, Formula Smiles, CAS, Source MPT, MPT °C, Source HFUS, HFUS J/mol
    Solvents = pd.read_excel('Adenventure\Handler\MixComp\Benchmark\Data\Molecules.xlsx', sheet_name= 'Solvents', index_col ="Material")
    ...
    if model == 'UNIFAC':
        ...
        path_empty = 'Adenventure\Handler\MixComp\Benchmark\Data\StructuralInformation_'
...
def load_Database(name_group, model):
    ...
    database = {}
    path_empty = 'Adenventure\Handler\MixComp\Benchmark\Data\database_'
    path_to_ctc2 = '.xlsx'
